WITH spending AS (
    SELECT
        user_id,  AVG(CAST(REPLACE(amount, '$', '') AS DOUBLE)) AS avg_spent
        FROM transactions_arrow
        GROUP BY user_id
),
credit_info AS (
    SELECT 
        c.user_id, c.num_cards_issued,  AVG(CAST(REPLACE(c.credit_limit, '$', '') AS DOUBLE)) AS avg_credit_limit
        FROM cards c
...
FROM credit_info ci
LEFT JOIN spending ON ci.user_id = s.user_id
